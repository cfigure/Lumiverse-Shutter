...
- **Show Prompt in Lightbox** — When on (off by default), opening a Shutter image in the native image viewer shows a compact **Prompt** pill directly beneath the image, with **Copy** and **View** actions when the generation prompt (and negative prompt) can be read from provider metadata embedded in the image. **View** expands the prompt into a panel below the image — sized relative to the image and screen so the artwork stays the focus — and **Collapse** returns to the pill; the **✕** hides it until the viewer is reopened.
